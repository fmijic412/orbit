...
/**
 * Manages a small set of red cubes that roam the arena along straight paths,
 * bouncing off the arena edges. They never leave the bounds, so the player can
 * always read and dodge them.
 *
...
export class Hazards {
  ...
  /** Re-seeds every hazard's position and heading (used on round restart). */
  reset(): void {
  ...
  /** Moves each hazard, bouncing it back inside the arena at the edges. */
  update(dt: number): void {
    ...
    for (let i = 0; i < this.cubes.length; i++) {
      ...
      cube.position.x += vel.x * dt;
      cube.position.z += vel.z * dt;
